...
term_powerset <- function(search_terms) {
  ...
  rapply(mapply(rje::powerSet, search_terms), function(x) { if(length(x) == 0) { return(c("")) } else { return(x) } }, how = "replace")
}
...
recursive_cartesian_product <- function(powersets, index = 1) {
  ...
  if (index+1 ==  length(powersets[1,])) {
    ...
    out <- two_dimentional_cartesian_product(powersets[,index], recursive_cartesian_product(powersets, index+1))
  }
