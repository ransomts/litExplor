...
create_compare_graph <- function(first_set, second_set) {
  first_formatted <- mapply(function(x) {
    paste(unlist(x), collapse = " or ")
  }, format_product_into_string(first_set))

  first_formatted[[1]] <- empty_space

  second_formatted <- mapply(function(x) {
    paste(unlist(x), collapse = " or ")
  }, format_product_into_string(second_set))
...
create_heatmap <- function(explor, setA, setB) {
  ...
  heat <- explor
  heat %<>% summarize(set_a = paste(!!!set_a_syms), set_b = paste(!!!set_b_syms), eric)
  print(heat)
  heat %<>% mutate(
    set_a = translate_binary_to_label(set_a, list(setA)),
    set_b = translate_binary_to_label(set_b, list(setB))
  )
  heat_plot <- heat %>% ggplot(aes(set_a, set_b, fill = eric)) +
    geom_tile() +
    labs(x = NULL, y = NULL) +
    theme(
