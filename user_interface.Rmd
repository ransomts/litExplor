...
search_eric_from_products <- function(formatted_products, options) {
  ...
  map(function(x) { create_eric_url(x, rows = 20, fields = options) })
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      textAreaInput("terms", "Terms", value = ""),
    ...
    mainPanel(
      ...
      # Output: Tabset w/ comparison, summary
...
server <- function(input, output) {
  ...
  observeEvent(input$term_dropdown, {
    ...
    print(num_found_for_terms)
    
    if (all(input$term_dropdown != "")) {
    
    terms_list <- mapply(function(x) { paste(x, collapse = ", ") }, format_terms_input_to_list(input$terms))
    ...
    output$compare <- renderUI({ selectInput(
      "compare",
      "",
    ...
    formatted_products <- split_terms_string(input$term_dropdown)
    ...
    print(readable_terms)
    
    terms_df <- as.matrix(num_found_for_terms)
    
    print(terms_df)
    
    randomData <- hasseDiagram::generateRandomData(15, 2, 0.5)
    
    print(randomData)
    
    print(as.matrix(terms_df))
    
    output$summary <- renderPlot({ hasseDiagram::hasse(as.matrix(terms_df)) }, height = 100, width = 100)
      ...
      output$placeholder <- renderText({"Select A First Term Set First"})
    }
  })
  
  
  observeEvent(input$compare, {
    if (all(input$compare != "")) {
  ...
  observeEvent(input$search, {
    ...
    # TODO: Implement a summary mechanism
    output$summary <- renderText(paste(input$terms))
    ...
    eric_responses <- search_eric_from_products(formatted_products, options)
