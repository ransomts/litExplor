...
make_queries_for_group <- function(group) {
  ...
  combine_augmented_sets <- function(set_a, set_b) {
    ...
    combined_acc <- bind_cols(set_a %>% slice(0) %>% select(-query),
                              set_b %>% slice(0))
    ...
    for (a_slice_index in 1:nrow(set_a)) {
      ...
      for (b_slice_index in 1:nrow(set_b)) {
        ...
        if (a_query == "" || b_query == "") {
          combined_query <- paste0(select(a_slice, query),
                                   select(b_slice, query))
        }
        ...
        combined_acc %<>% add_row(bind_cols(a_terms,
                                            b_terms,
...
test_explor_a <- group_to_explor(make_queries_for_group(test_group_a))
test_explor_b <- group_to_explor(make_queries_for_group(test_group_b))
test_explor_c <- group_to_explor(make_queries_for_group(test_group_c))
```
...
test_eric_count <- get_eric_count("A AND B")
```

## Make ERIC explor data

```{r}

#user_input_explor <- parsed_user_input %>% set_to_group %>% make_queries_for_group %>% group_to_explor

if (file.exists("cache/test_eric_explor_a.Rds")) {
  "test_eric_explor_a" <- read_rds("cache/test_eric_explor_a.Rds")
} else {
  test_eric_explor_a <- explor_a %>% mutate(eric = get_eric_count(query))
  test_eric_explor_a %<>% unnest(eric)
  write_rds(test_eric_explor_a, "cache/test_eric_explor_a.Rds")
}

if (file.exists("cache/test_eric_explor_b.Rds")) {
  "test_eric_explor_b" <- read_rds("cache/test_eric_explor_b.Rds")
} else {
  test_eric_explor_b <- explor_b %>% mutate(eric = get_eric_count(query))
  test_eric_explor_b %<>% unnest(eric)
  write_rds(test_eric_explor_b, "cache/test_eric_explor_b.Rds")
}

if (file.exists("cache/test_eric_explor_c.Rds")) {
  "test_eric_explor_c" <- read_rds("cache/test_eric_explor_c.Rds")
} else {
  test_eric_explor_c <- explor_c %>% mutate(eric = get_eric_count(query))
  test_eric_explor_c %<>% unnest(eric)
  write_rds(test_eric_explor_c, "cache/test_eric_explor_c.Rds")
}
```
